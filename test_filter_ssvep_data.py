...
Describe how the spectra change at each stage and why.
    ...
    In the power spectrum for the raw data, many peaks are observed, including peaks at the stimuli frequencies and their harmonics as well as noise and distinct artifacts (like the 50Hz powerline artifact). In general, the stimuli frequencies are most prominent in the occipital channels, as the occipital lobe is the primary location of visual processing. When the (15Hz) filter is applied to the data, a single peak at 15Hz is observed for both the 12Hz and 15Hz trials. The bandpass filter reduces the prominence (reflected by power) of the signal's frequencies outside of the passband, highlighting why the peak is observed at 15Hz, as, in the nearby frequencies, 15Hz peaks in the raw data power spectrum as well. At lower filter orders, a notable peak also occurs at 12Hz, though this peak disappears at higher filter orders, indicative of the sharpened cutoff effect that occurs at higher orders. Within the filtered spectra, no peaks are observed at the known harmonic or artifact frequencies. Overall, this filter reduces the power of signals outside of the bandpass frequency window, and as filter order increases, the stronger that effect becomes.
    
    2. There are numerous ways in which the filtered spectra for the 12Hz and 15Hz trials may be compared to the raw data's spectra. First, there are multiple ways in which normalization may be performed; namely, each aspect of the data may be normalized to itself (i.e. the raw data normalized to the maximum within the raw data power spectrum, filtered data to the maximum within the filtered spectrum, etc.) or to the raw data's spectrum. The vertical axis of the power spectrum (dB) is profoundly affected by the normalization factor. If a power spectrum is normalized with its maximum value, then the peak value will be 0dB at the frequency of the maximum. When the spectra are normalized to themselves, the power for each set is less comparable to the other datasets but highlights important features related to each manipulation. For example, normalizing the power spectra of the filtered data to itself presents distinct power differences between the 12Hz and 15Hz stimuli. At most frequencies, the 12Hz stimulus presents with a higher power than the 15Hz stimulus for the 15Hz filter (except at 15Hz, where there is a very high peak in power). The increased power for the 12Hz stimulus could be reflective of several signals in the data, including not only the 12Hz stimulus but the alpha waves (occur between 8 and 12Hz). Though the frequencies outside of 14-16Hz are largely filtered out (especially at higher orders), the signals still possess higher "baseline" power, which can be seen where the 12Hz peak in the unfiltered signal has a higher power than the 15Hz signal.
    
    In contrast, it is possible to observe the power spectra normalized to the raw data power spectrum. In fact, several of the trends that appear when the datasets are normalized to themselves change when normalized to a different factor, such as the raw spectrum. In channel Oz, the difference in the filtered power spectrum between the two signals mostly disappears, but the difference between the spectra for these stimuli in channel Fz increases. More importantly, utilizing this normalization factor allows for a more direct comparison across power spectra for the two stimuli. This phenomenon is demonstrated in two of the corresponding images, SSVEP_S1_frequency_content_15Hz_filter.png (Figure 1) and SSVEP_S1_frequency_content_15Hz_filter_raw_normalized.png (Figure 2). Both figures depict attenuation of frequencies outside of the passband, but only using a common normalization factor (Figure 2) is it seen that the filter preserves the signal strength of the stimulus (namely, the 15Hz stimulus for the 15Hz filter). Notably, it can be seen that the peak power in the filtered spectrum at 15Hz (channel Oz) is the same power at 15Hz as in the unfiltered data's spectrum. Thus, the relationship between the raw spectrum and filtered spectrum at each frequency is easily observed regarding how the filter changed the power directly, while normalizing each spectrum to itself shows the relative effect the filter had (making trends/shapes more signficant, whereas exact values may be more informative with using the raw data normalization factor and preservation of signal strength).
    
    Because the difference observed between the 12Hz and 15Hz trials on channel Oz only occurred in Figure 1 (with the 12Hz trials mostly exhibiting higher power than the 15Hz trials - there is little to no difference observed in Figure 2), it can be deduced that the difference is actually an artifact of the specific normalization factor. Thus, it is likely that there is minimal true significance of the differences seen in Figure 1, though the relative power (only with respect to the trial itself) is depicted slightly better in Figure 1.
